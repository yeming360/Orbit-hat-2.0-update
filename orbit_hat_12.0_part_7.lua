print("🟢 Hat Orbit v9.9.8 loading...")
...
--  SETTINGS
-- ═══════════════════════════════════════════════════════
-- OUTER RING 1 (Main + Shoot Out)
local DISTANCE, ORBIT_RADIUS, ORBIT_SPEED, ORBIT_MODE = 5, 3, 60, "Y"
local HEIGHT_OFFSET, SPIN_SPEED, ORBIT_ROTATION = 2, 1, 0

-- OUTER RING 2 (with FULL 3-AXIS WING MODE)
local USE_OUTER2, OUTER2_COUNT, OUTER2_MODE = false, 4, "X"
local OUTER2_DISTANCE, OUTER2_RADIUS, OUTER2_SPEED = 5, 3, 60
local OUTER2_HEIGHT, OUTER2_SPIN, OUTER2_ROTATION = 2, 1, 0

-- WING MODE (Outer Ring 2 only) - FULL 3 AXIS
...
-- OUTER RING 3
local USE_OUTER3, OUTER3_COUNT, OUTER3_MODE = false, 4, "Y"
local OUTER3_DISTANCE, OUTER3_RADIUS, OUTER3_SPEED = 5, 4, 60
local OUTER3_HEIGHT, OUTER3_SPIN, OUTER3_ROTATION = 2, 1, 0

-- OUTER RING 4
local USE_OUTER4, OUTER4_COUNT, OUTER4_MODE = false, 4, "Z"
local OUTER4_DISTANCE, OUTER4_RADIUS, OUTER4_SPEED = 6, 5, 45
local OUTER4_HEIGHT, OUTER4_SPIN, OUTER4_ROTATION = 3, 1, 0

-- SHOOT OUT (ONLY Outer Ring 1)
local USE_SHOOT_OUT, SHOOT_OUT_RANGE, SHOOT_OUT_SPEED = false, 15, 120
local SHOOT_OUT_ORBIT_SPEED, USE_SHOOT_OUT_ORBIT = 60, true

-- INNER RING
local USE_INNER_RING, INNER_COUNT, INNER_MODE, INNER_ORBIT_MODE = true, 4, "DoubleLazer", "Y"
local INNER_DISTANCE, INNER_ORBIT_RADIUS, INNER_ORBIT_SPEED = 2, 1.5, 120
local INNER_HEIGHT_OFFSET, INNER_SPIN_SPEED = 2, 2

local INNER_LAZER_DISTANCE, INNER_LAZER_HEIGHT, INNER_LAZER_RANGE = 3, 2, 5
local INNER_LAZER_SPEED, INNER_LAZER_RADIUS, INNER_LAZER_ORBIT_SPEED, INNER_BEAM_GAP = 180, 1.2, 400, 2

local FB_ORBIT_SPEED, FB_Y_SPIN_SPEED, FB_DISTANCE, FB_HEIGHT = 150, 100, 2.5, 1.5
local FB_SIZE, FB_RANGE, FB_SHOOT_SPEED = 3.5, 5, 200

local DS_ORBIT_SPEED, DS_Y_SPIN_SPEED, DS_DISTANCE, DS_HEIGHT = 150, 100, 3, 2
local DS_SIZE, DS_CENTER_SPEED, DS_CENTER_RADIUS = 2.5, 60, 3

-- SHIELD
local USE_SHIELD, SHIELD_DISTANCE, SHIELD_RADIUS, SHIELD_SPEED = false, 3, 2.5, 90
local SHIELD_HEIGHT, SHIELD_LEFT_INDEX, SHIELD_RIGHT_INDEX = 1, 1, 2

-- GENERAL
local SPIN_MODE, MAGNET_ENABLED, SMOOTHNESS, MAX_HATS, GUI_SCALE = "Y", true, 12, 22, 1.0
local GIFT_ENABLED, GIFT_TARGET_NAME, isActive = false, "", true

-- SPLIT
local USE_SPLIT, SPLIT_RATIO = false, 0.5
local USE_OUTER2_SPLIT, OUTER2_SPLIT_RATIO = false, 0.5
local USE_OUTER3_SPLIT, OUTER3_SPLIT_RATIO = false, 0.5
local USE_OUTER4_SPLIT, OUTER4_SPLIT_RATIO = false, 0.5
...
-- DLAZER HOLD
local USE_DLAZER_HOLD, DLAZER_LEFT_INDEX, DLAZER_RIGHT_INDEX = false, 1, 2
local DLAZER_DISTANCE, DLAZER_HEIGHT, DLAZER_RANGE = 10, 2, 10
local DLAZER_SHOOT_SPEED, DLAZER_BEAM_RADIUS, DLAZER_ORBIT_SPEED = 400, 3, 180
local DLAZER_LEFT_OFFSET, DLAZER_RIGHT_OFFSET = 0.7, 0.7
local DLAZER_ROT_X, DLAZER_ROT_Y, DLAZER_ROT_Z = 0, 0, 0

-- FIREBALL HOLD
local USE_FIREBALL_HOLD, FIREBALL_LEFT_INDEX, FIREBALL_RIGHT_INDEX = false, 1, 2
local FIREBALL_DISTANCE, FIREBALL_HEIGHT, FIREBALL_ORBIT_SPEED = 5, 0, 180
local FIREBALL_Y_SPIN_SPEED, FIREBALL_SPHERE_SIZE, FIREBALL_RANGE = 120, 4, 8
local FIREBALL_SHOOT_SPEED, FIREBALL_LEFT_OFFSET, FIREBALL_RIGHT_OFFSET = 200, 0.7, 0.7
local FIREBALL_ROT_X, FIREBALL_ROT_Y, FIREBALL_ROT_Z = 0, 0, 0
...
local heldHats = {}
local dlazerHats, dlazerShootAngle, dlazerOrbitAngle = {}, 0, 0
local fireballHats, fireballHoldZAngle, fireballHoldYAngle, fireballHoldPulse = {left={}, right={}}, 0, 0, 0
local shootOutAngle = 0

-- WING MODE STATE (3 axes)
local wingAngleX, wingAngleY, wingAngleZ = 0, 0, 0
local wingDirX, wingDirY, wingDirZ = 1, 1, 1

-- HELPERS
local function waitForHRP()
    while not hrp or not hrp.Parent do hrp = chr and chr:FindFirstChild("HumanoidRootPart"); if not hrp then task.wait() end end
end
local function isWearable(inst) return inst:IsA("Accessory") or inst:IsA("Hat") end
...
local function setupHatOrbit(accessory)
    ...
    pcall(function() if not handle.Anchored then handle:SetNetworkOwner(plr) end end)
    ...
    connections[handle]=accessory.AncestryChanged:Connect(function(_,p) if p~=chr then cleanupHat(handle) end end)
...
local function captureAllowedHats()
    if not chr then return end; waitForHRP()
    for _,c in ipairs(chr:GetChildren()) do if isWearable(c) then local h=c:FindFirstChild("Handle"); if h and not orbitData[h] then setupHatOrbit(c) end end end
end
...
local function resolveAnchor()
    if GIFT_ENABLED and GIFT_TARGET_NAME~="" then local tp=Players:FindFirstChild(GIFT_TARGET_NAME); local tc=tp and tp.Character; local th=tc and tc:FindFirstChild("HumanoidRootPart"); if th then return th end end; return hrp
end
local function getHatHandlesSorted() local s={} for h in pairs(orbitData) do table.insert(s,h) end; table.sort(s,function(a,b) return a.Name<b.Name end); return s end
...
local function setDLazerHoldEnabled(enabled)
    ...
    if not chr or not chr:FindFirstChild("LeftHand") or not chr:FindFirstChild("RightHand") then USE_DLAZER_HOLD=false return end
    local s=getHatHandlesSorted()
...
local function setFireballHoldEnabled(enabled)
    USE_FIREBALL_HOLD=enabled; ensureFireballStructure()
    ...
    if not chr or not chr:FindFirstChild("LeftHand") or not chr:FindFirstChild("RightHand") then USE_FIREBALL_HOLD=false return end
    local s=getHatHandlesSorted()
...
local function updateHoldSlot(slotIdx)
    ...
    if not slot.USE then return end; if not (hats.left or hats.right) then return end; if not chr then return end
...
local function updateDLazerHold(dt)
    if not USE_DLAZER_HOLD then return end; if not (dlazerHats.left or dlazerHats.right) then return end; if not chr then return end
    local lh=chr:FindFirstChild("LeftHand"); local rh=chr:FindFirstChild("RightHand"); if not lh or not rh then return end
...
local function updateFireballHold(dt)
    if not USE_FIREBALL_HOLD then return end; ensureFireballStructure(); if not chr then USE_FIREBALL_HOLD=false return end
    local lh=chr:FindFirstChild("LeftHand"); local rh=chr:FindFirstChild("RightHand"); if not lh or not rh then USE_FIREBALL_HOLD=false return end
...
-- WING MODE UPDATE (3 AXES)
local function updateWingMode(dt)
    if not USE_WING or not USE_OUTER2 then return end
    
    -- X Axis
    if WING_MAX_X ~= WING_MIN_X then
        local minRad = math.rad(WING_MIN_X)
        local maxRad = math.rad(WING_MAX_X)
        wingAngleX = wingAngleX + (math.rad(WING_SPEED_X) * dt * wingDirX)
        if wingAngleX >= maxRad then wingAngleX = maxRad; wingDirX = -1
        elseif wingAngleX <= minRad then wingAngleX = minRad; wingDirX = 1 end
    end
    
    -- Y Axis
    if WING_MAX_Y ~= WING_MIN_Y then
        local minRad = math.rad(WING_MIN_Y)
        local maxRad = math.rad(WING_MAX_Y)
        wingAngleY = wingAngleY + (math.rad(WING_SPEED_Y) * dt * wingDirY)
        if wingAngleY >= maxRad then wingAngleY = maxRad; wingDirY = -1
        elseif wingAngleY <= minRad then wingAngleY = minRad; wingDirY = 1 end
    end
    
    -- Z Axis
    if WING_MAX_Z ~= WING_MIN_Z then
        local minRad = math.rad(WING_MIN_Z)
        local maxRad = math.rad(WING_MAX_Z)
        wingAngleZ = wingAngleZ + (math.rad(WING_SPEED_Z) * dt * wingDirZ)
        if wingAngleZ >= maxRad then wingAngleZ = maxRad; wingDirZ = -1
        elseif wingAngleZ <= minRad then wingAngleZ = minRad; wingDirZ = 1 end
    end
end

-- RENDER LOOP
local TWO_PI=math.pi*2; local V3_ZERO=Vector3.zero; local CF_ID=CFrame.identity
...
RunService.RenderStepped:Connect(function(dt)
    if not isActive then return end; if not hrp or not hrp.Parent then chr=plr.Character; hrp=chr and chr:FindFirstChild("HumanoidRootPart"); return end
    outerOrbitAngle=(outerOrbitAngle+math.rad(ORBIT_SPEED)*dt)%TWO_PI
    outerSpinAngle=(outerSpinAngle+math.rad(SPIN_SPEED*60)*dt)%TWO_PI
    outerSplitAngle=(outerSplitAngle-math.rad(ORBIT_SPEED)*dt)%TWO_PI
    if USE_OUTER2 then
        outer2OrbitAngle=(outer2OrbitAngle+math.rad(OUTER2_SPEED)*dt)%TWO_PI
        if not USE_WING then outer2SpinAngle=(outer2SpinAngle+math.rad(OUTER2_SPIN*60)*dt)%TWO_PI end
        outer2SplitAngle=(outer2SplitAngle-math.rad(OUTER2_SPEED)*dt)%TWO_PI
        updateWingMode(dt)
    end
    if USE_OUTER3 then outer3OrbitAngle=(outer3OrbitAngle+math.rad(OUTER3_SPEED)*dt)%TWO_PI; outer3SpinAngle=(outer3SpinAngle+math.rad(OUTER3_SPIN*60)*dt)%TWO_PI; outer3SplitAngle=(outer3SplitAngle-math.rad(OUTER3_SPEED)*dt)%TWO_PI end
    if USE_OUTER4 then outer4OrbitAngle=(outer4OrbitAngle+math.rad(OUTER4_SPEED)*dt)%TWO_PI; outer4SpinAngle=(outer4SpinAngle+math.rad(OUTER4_SPIN*60)*dt)%TWO_PI; outer4SplitAngle=(outer4SplitAngle-math.rad(OUTER4_SPEED)*dt)%TWO_PI end
    if USE_INNER_RING then
        innerOrbitAngle=(innerOrbitAngle+math.rad(INNER_ORBIT_SPEED)*dt)%TWO_PI; innerSpinAngle=(innerSpinAngle+math.rad(INNER_SPIN_SPEED*60)*dt)%TWO_PI
        if INNER_MODE=="Lazer" or INNER_MODE=="DoubleLazer" then innerLazerAngle=(innerLazerAngle+math.rad(INNER_LAZER_SPEED)*dt)%TWO_PI; lazerOrbitAngle=(lazerOrbitAngle+math.rad(INNER_LAZER_ORBIT_SPEED)*dt)%TWO_PI end
        if INNER_MODE=="Fireball" then fireballZAngle=(fireballZAngle+math.rad(FB_ORBIT_SPEED)*dt)%TWO_PI; fireballYAngle=(fireballYAngle+math.rad(FB_Y_SPIN_SPEED)*dt)%TWO_PI; fireballPulse=(fireballPulse+math.rad(FB_SHOOT_SPEED)*dt)%TWO_PI end
        if INNER_MODE=="DoubleStar" then dsZAngle=(dsZAngle+math.rad(DS_ORBIT_SPEED)*dt)%TWO_PI; dsYAngle=(dsYAngle+math.rad(DS_Y_SPIN_SPEED)*dt)%TWO_PI; dsCenterAngle=(dsCenterAngle+math.rad(DS_CENTER_SPEED)*dt)%TWO_PI end
    end
    shootOutAngle=(shootOutAngle+math.rad(SHOOT_OUT_SPEED)*dt)%TWO_PI
    updateShield(dt); for i=1,10 do updateHoldSlot(i) end; updateDLazerHold(dt); updateFireballHold(dt)
    local availableIndices={} for i=1,#orbitParts do local h=handles[i]; if h and not heldHats[h] then table.insert(availableIndices,i) end end
    ...
    local u1,v1=uv(ORBIT_MODE); local u2,v2=uv(OUTER2_MODE); local u3,v3=uv(OUTER3_MODE); local u4,v4=uv(OUTER4_MODE); local uIn,vIn=uv(INNER_ORBIT_MODE)
    local spinVec=(SPIN_MODE=="X") and Vector3.new(10,0,0) or (SPIN_MODE=="Y") and Vector3.new(0,10,0) or Vector3.new(0,0,10)
    local innerCap=USE_INNER_RING and math.min(INNER_COUNT,availCount) or 0; local rem=availCount-innerCap
    ...
    local o4c=USE_OUTER4 and math.min(OUTER4_COUNT,rem) or 0; local o1c=math.max(rem-o4c,0)
    local alpha=1-math.exp(-SMOOTHNESS*10*dt)
    for availIdx,i in ipairs(availableIndices) do
        local op=orbitParts[i]; local h=handles[i]; local d=orbitData[h]; if not d then continue end
        ...
        if isIn then
            ...
            if INNER_MODE=="Ring" then local cp=a.Position+b*INNER_DISTANCE+u*INNER_HEIGHT_OFFSET; local ang=(2*math.pi/innerCap)*(availIdx-1)+innerOrbitAngle; tp=cp+uIn*math.cos(ang)*INNER_ORBIT_RADIUS+vIn*math.sin(ang)*INNER_ORBIT_RADIUS
            elseif INNER_MODE=="Lazer" then local osc=math.sin(innerLazerAngle)*INNER_LAZER_RANGE; local bd=INNER_LAZER_DISTANCE+osc; if availIdx==innerCap then local ra=lazerOrbitAngle; local ro=r*math.cos(ra)*INNER_LAZER_RADIUS+u*math.sin(ra)*INNER_LAZER_RADIUS; tp=a.Position+b*bd+u*INNER_LAZER_HEIGHT+ro else local so=(availIdx-1)*0.03; local sp=(availIdx-1)*0.02; tp=a.Position+b*(bd+so)+u*(INNER_LAZER_HEIGHT+sp) end
            elseif INNER_MODE=="DoubleLazer" then local half=math.ceil(innerCap/2); local isBeam1=availIdx<=half; local beamIdx=isBeam1 and (availIdx-1) or (availIdx-half-1); local beamCount=isBeam1 and half or (innerCap-half); local isTip=(isBeam1 and availIdx==half) or (not isBeam1 and availIdx==innerCap); local osc=math.sin(innerLazerAngle)*INNER_LAZER_RANGE; local bd=INNER_LAZER_DISTANCE+osc; local gap=INNER_BEAM_GAP; local sideOffset=isBeam1 and (-gap/2) or (gap/2); local beamCenter=a.Position+b*bd+u*INNER_LAZER_HEIGHT+r*sideOffset; if isTip then local ra=lazerOrbitAngle; local ro=r*math.cos(ra)*INNER_LAZER_RADIUS+u*math.sin(ra)*INNER_LAZER_RADIUS; tp=beamCenter+ro else local trailIndex=beamCount-beamIdx-1; local trailDist=trailIndex*0.8; tp=beamCenter-b*trailDist end
            elseif INNER_MODE=="DoubleStar" then local sc=math.ceil(innerCap/2); local isA=availIdx<=sc; local gi=isA and (availIdx-1) or (availIdx-sc-1); local gs=math.max(isA and sc or (innerCap-sc),1); local bc=a.Position+b*DS_DISTANCE+u*DS_HEIGHT; local sa=dsCenterAngle+(isA and 0 or math.pi); local scp=bc+r*math.cos(sa)*DS_CENTER_RADIUS+b*math.sin(sa)*DS_CENTER_RADIUS; local ph=(2*math.pi/gs)*gi; local za=dsZAngle+ph; local lx=math.sin(za)*DS_SIZE; local ly=math.cos(za)*DS_SIZE; local cr=math.cos(dsYAngle); local sr=math.sin(dsYAngle); local wz=lx*cr; local wx=lx*sr; tp=scp+b*wz+r*wx+u*ly
            else local ro=math.sin(fireballPulse)*FB_RANGE; local bd=FB_DISTANCE+ro; local ph=(2*math.pi/innerCap)*(availIdx-1); local za=fireballZAngle+ph; local lx=math.sin(za)*FB_SIZE; local ly=math.cos(za)*FB_SIZE; local cr=math.cos(fireballYAngle); local sr=math.sin(fireballYAngle); local wz=lx*cr; local wx=lx*sr; tp=a.Position+b*(bd+wz)+r*wx+u*(FB_HEIGHT+ly) end
        else
            ...
            if availOuterIdx<=o2c then ringCount=o2c; ringIdx=availOuterIdx-1; ringAngle=outer2OrbitAngle; ringSplitAngle=outer2SplitAngle; ringSplitRatio=OUTER2_SPLIT_RATIO; ringRot=math.rad(OUTER2_ROTATION); cp=a.Position+b*OUTER2_DISTANCE+u*OUTER2_HEIGHT; axisU,axisV=u2,v2; ringRadius=OUTER2_RADIUS; useSplit=USE_OUTER2_SPLIT; splitRatio=OUTER2_SPLIT_RATIO; useShootOut=false
            elseif availOuterIdx<=o2c+o3c then ringCount=o3c; ringIdx=availOuterIdx-o2c-1; ringAngle=outer3OrbitAngle; ringSplitAngle=outer3SplitAngle; ringSplitRatio=OUTER3_SPLIT_RATIO; ringRot=math.rad(OUTER3_ROTATION); cp=a.Position+b*OUTER3_DISTANCE+u*OUTER3_HEIGHT; axisU,axisV=u3,v3; ringRadius=OUTER3_RADIUS; useSplit=USE_OUTER3_SPLIT; splitRatio=OUTER3_SPLIT_RATIO; useShootOut=false
            elseif availOuterIdx<=o2c+o3c+o4c then ringCount=o4c; ringIdx=availOuterIdx-o2c-o3c-1; ringAngle=outer4OrbitAngle; ringSplitAngle=outer4SplitAngle; ringSplitRatio=OUTER4_SPLIT_RATIO; ringRot=math.rad(OUTER4_ROTATION); cp=a.Position+b*OUTER4_DISTANCE+u*OUTER4_HEIGHT; axisU,axisV=u4,v4; ringRadius=OUTER4_RADIUS; useSplit=USE_OUTER4_SPLIT; splitRatio=OUTER4_SPLIT_RATIO; useShootOut=false
            else ringCount=o1c; ringIdx=availOuterIdx-o2c-o3c-o4c-1; ringAngle=outerOrbitAngle; ringSplitAngle=outerSplitAngle; ringSplitRatio=SPLIT_RATIO; ringRot=math.rad(ORBIT_ROTATION); cp=a.Position+b*DISTANCE+u*HEIGHT_OFFSET; axisU,axisV=u1,v1; ringRadius=ORBIT_RADIUS; useSplit=USE_SPLIT; splitRatio=SPLIT_RATIO; useShootOut=USE_SHOOT_OUT; shootOutRange=SHOOT_OUT_RANGE; shootOutSpeed=SHOOT_OUT_SPEED; shootOutOrbitSpeed=SHOOT_OUT_ORBIT_SPEED end
            
            if useShootOut then local hatAngle=(2*math.pi/ringCount)*ringIdx+ringAngle; local shootOutProgress=(math.sin(shootOutAngle)+1)*0.5; local radialDistance=ringRadius+shootOutProgress*shootOutRange; local orbitAngle=ringAngle; if USE_SHOOT_OUT_ORBIT then orbitAngle=ringAngle+(shootOutOrbitSpeed/60)*tick()%TWO_PI end; local dirX=math.cos(hatAngle); local dirY=math.sin(hatAngle); if USE_SHOOT_OUT_ORBIT then local cosO=math.cos(orbitAngle-ringAngle); local sinO=math.sin(orbitAngle-ringAngle); local rotatedX=dirX*cosO-dirY*sinO; local rotatedY=dirX*sinO+dirY*cosO; dirX,dirY=rotatedX,rotatedY end; tp=cp+axisU*(dirX*radialDistance)+axisV*(dirY*radialDistance)
            ...
            -- WING MODE: Apply 3-axis rotation to Outer Ring 2 hats
            if isO2 and USE_WING then
                local wingCFrame = CFrame.Angles(wingAngleX, wingAngleY, wingAngleZ)
                tp = (tp - cp) + cp
        ...
        if not (isO2 and USE_WING) then
            if MAGNET_ENABLED then local lk=CF_LOOKAT(tp,a.Position); local sa=isIn and innerSpinAngle or (isO2 and outer2SpinAngle or (isO3 and outer3SpinAngle or (isO4 and outer4SpinAngle or outerSpinAngle))); local sc=CF_ID; if SPIN_MODE=="Y" then sc=CF_ANGLES(0,sa,0) elseif SPIN_MODE=="X" then sc=CF_ANGLES(sa,0,0) elseif SPIN_MODE=="Z" then sc=CF_ANGLES(0,0,sa) end; op.CFrame=lk*sc; if d.angularVel then d.angularVel.AngularVelocity=V3_ZERO end else op.CFrame=CF_NEW(tp); if d.angularVel then d.angularVel.AngularVelocity=spinVec end end
        end
    end
end)

-- CHARACTER HOOKS
local function onChar(c) chr=c; hrp=c:WaitForChild("HumanoidRootPart"); removeAllHats(); task.wait(0.3); captureAllowedHats(); c.ChildAdded:Connect(function(ch) task.wait(); if isWearable(ch) then setupHatOrbit(ch) end end) end
if plr.Character then onChar(plr.Character) end; plr.CharacterAdded:Connect(onChar)

-- GUI (Complete with 3-Axis Wing Mode)
local function createGUI()
    local screenGui=Instance.new("ScreenGui"); screenGui.Name="HatOrbitUI"; screenGui.ResetOnSpawn=false; screenGui.Parent=plr:WaitForChild("PlayerGui")
    local main=Instance.new("Frame",screenGui); main.Size=UDim2.new(0,280,0,600); main.Position=UDim2.new(0.5,-140,0.5,-300); main.BackgroundColor3=Color3.fromRGB(18,18,22); main.BackgroundTransparency=0.08; main.BorderSizePixel=0; main.Active=true; main.Draggable=true; Instance.new("UICorner",main).CornerRadius=UDim.new(0,10)
    local uiScale=Instance.new("UIScale",main); uiScale.Scale=GUI_SCALE
    local bar=Instance.new("Frame",main); bar.Size=UDim2.new(1,0,0,34); bar.BackgroundColor3=Color3.fromRGB(28,28,35); bar.BorderSizePixel=0; bar.ZIndex=2; Instance.new("UICorner",bar).CornerRadius=UDim.new(0,10)
    local title=Instance.new("TextLabel",bar); title.Size=UDim2.new(1,-50,1,0); title.Position=UDim2.new(0,14,0,0); title.BackgroundTransparency=1; title.Text="🔥 Hat Orbit v9.9.8"; title.TextColor3=Color3.new(1,1,1); title.Font=Enum.Font.GothamBold; title.TextSize=13; title.TextXAlignment=Enum.TextXAlignment.Left; title.ZIndex=3
    local powerBtn=Instance.new("TextButton",bar); powerBtn.Size=UDim2.new(0,34,0,24); powerBtn.Position=UDim2.new(1,-68,0.5,-12); powerBtn.Text="⏻"; powerBtn.BackgroundColor3=Color3.fromRGB(60,200,60); powerBtn.TextColor3=Color3.new(1,1,1); powerBtn.Font=Enum.Font.GothamBold; powerBtn.TextSize=16; powerBtn.ZIndex=3; Instance.new("UICorner",powerBtn).CornerRadius=UDim.new(0,5)
    local closeBtn=Instance.new("TextButton",bar); closeBtn.Size=UDim2.new(0,24,0,24); closeBtn.Position=UDim2.new(1,-30,0.5,-12); closeBtn.Text="✕"; closeBtn.BackgroundColor3=Color3.fromRGB(200,45,45); closeBtn.TextColor3=Color3.new(1,1,1); closeBtn.Font=Enum.Font.GothamBold; closeBtn.TextSize=13; closeBtn.ZIndex=3; Instance.new("UICorner",closeBtn).CornerRadius=UDim.new(0,5); closeBtn.MouseButton1Click:Connect(function() isActive=false; removeAllHats(); screenGui:Destroy() end)
    local scrollFrame=Instance.new("ScrollingFrame",main); scrollFrame.Position=UDim2.new(0,0,0,34); scrollFrame.Size=UDim2.new(1,0,1,-34); scrollFrame.BackgroundTransparency=1; scrollFrame.BorderSizePixel=0; scrollFrame.ScrollBarThickness=4; scrollFrame.ScrollBarImageColor3=Color3.fromRGB(100,100,110); scrollFrame.VerticalScrollBarInset=Enum.ScrollBarInset.ScrollBar; scrollFrame.ZIndex=2; Instance.new("UICorner",scrollFrame).CornerRadius=UDim.new(0,6)
    local content=Instance.new("Frame",scrollFrame); content.Size=UDim2.new(1,-8,0,0); content.BackgroundTransparency=1; content.ZIndex=2
    local y=10; local PAD=12
    local function addModeRow(label,modes,getCur,onSel)
        local l=Instance.new("TextLabel",content); l.Size=UDim2.new(1,-PAD*2,0,14); l.Position=UDim2.new(0,PAD,0,y); l.BackgroundTransparency=1; l.Text=label; l.TextColor3=Color3.fromRGB(190,190,200); l.Font=Enum.Font.GothamMedium; l.TextSize=11; l.TextXAlignment=Enum.TextXAlignment.Left; y=y+16
        local r=Instance.new("Frame",content); r.Size=UDim2.new(1,-PAD*2,0,32); r.Position=UDim2.new(0,PAD,0,y); r.BackgroundTransparency=1
        local btns={}; local function rf() for k,b in pairs(btns) do b.BackgroundColor3=(k==getCur()) and Color3.fromRGB(70,130,240) or Color3.fromRGB(40,40,48) end end
        local tg=(#modes-1)*4; local bw=((280-PAD*2)-tg)/#modes
        for i,m in ipairs(modes) do local b=Instance.new("TextButton",r); b.Size=UDim2.new(0,bw,1,0); b.Position=UDim2.new(0,(i-1)*(bw+4),0,0); b.Text=m.text; b.Font=Enum.Font.GothamBold; b.TextSize=9; b.TextColor3=Color3.new(1,1,1); b.BackgroundColor3=Color3.fromRGB(40,40,48); b.ZIndex=3; Instance.new("UICorner",b).CornerRadius=UDim.new(0,6); btns[m.key]=b; b.MouseButton1Click:Connect(function() onSel(m.key); rf() end) end; rf(); y=y+40
    end
    local function addSlider(label,minV,maxV,def,step,onCh)
        local r=Instance.new("Frame",content); r.Size=UDim2.new(1,-PAD*2,0,38); r.Position=UDim2.new(0,PAD,0,y); r.BackgroundTransparency=1
        local l=Instance.new("TextLabel",r); l.Size=UDim2.new(1,-44,0,16); l.BackgroundTransparency=1; l.Text=label; l.TextColor3=Color3.fromRGB(190,190,200); l.Font=Enum.Font.GothamMedium; l.TextSize=11; l.TextXAlignment=Enum.TextXAlignment.Left
        local vl=Instance.new("TextLabel",r); vl.Size=UDim2.new(0,44,0,16); vl.Position=UDim2.new(1,-44,0,0); vl.BackgroundTransparency=1; vl.Text=tostring(def); vl.TextColor3=Color3.fromRGB(140,200,255); vl.Font=Enum.Font.Code; vl.TextSize=11; vl.TextXAlignment=Enum.TextXAlignment.Right
        local b=Instance.new("TextBox",r); b.Size=UDim2.new(1,-44,0,20); b.Position=UDim2.new(0,0,0,16); b.BackgroundColor3=Color3.fromRGB(40,40,48); b.TextColor3=Color3.new(1,1,1); b.Font=Enum.Font.Code; b.TextSize=12; b.Text=tostring(def); Instance.new("UICorner",b).CornerRadius=UDim.new(0,6)
        local mn=Instance.new("TextButton",r); mn.Size=UDim2.new(0,18,0,18); mn.Position=UDim2.new(1,-44,0,18); mn.Text="−"; mn.BackgroundColor3=Color3.fromRGB(55,55,65); mn.TextColor3=Color3.new(1,1,1); mn.Font=Enum.Font.GothamBold; mn.TextSize=12; mn.ZIndex=5; Instance.new("UICorner",mn).CornerRadius=UDim.new(0,4)
        local pl=Instance.new("TextButton",r); pl.Size=UDim2.new(0,18,0,18); pl.Position=UDim2.new(1,-22,0,18); pl.Text="+"; pl.BackgroundColor3=Color3.fromRGB(55,55,65); pl.TextColor3=Color3.new(1,1,1); pl.Font=Enum.Font.GothamBold; pl.TextSize=12; pl.ZIndex=5; Instance.new("UICorner",pl).CornerRadius=UDim.new(0,4)
        local function ap(v) v=math.clamp(tonumber(v) or def,minV,maxV); local rd=math.floor(v*100+0.5)/100; b.Text=tostring(rd); vl.Text=tostring(rd); onCh(rd) end
        mn.MouseButton1Click:Connect(function() ap((tonumber(b.Text) or def)-step) end); pl.MouseButton1Click:Connect(function() ap((tonumber(b.Text) or def)+step) end); b.FocusLost:Connect(function() ap(b.Text) end); y=y+42
    end
    local function addTog(label,getSt,onTog,onCl,offCl)
        local r=Instance.new("Frame",content); r.Size=UDim2.new(1,-PAD*2,0,30); r.Position=UDim2.new(0,PAD,0,y); r.BackgroundTransparency=1
        local l=Instance.new("TextLabel",r); l.Size=UDim2.new(0,170,1,0); l.BackgroundTransparency=1; l.Text=label; l.TextColor3=Color3.fromRGB(190,190,200); l.Font=Enum.Font.GothamMedium; l.TextSize=11; l.TextXAlignment=Enum.TextXAlignment.Left
        local b=Instance.new("TextButton",r); b.Size=UDim2.new(0,70,1,0); b.Position=UDim2.new(1,-70,0,0); b.Font=Enum.Font.GothamBold; b.TextSize=11; b.TextColor3=Color3.new(1,1,1); b.ZIndex=3; Instance.new("UICorner",b).CornerRadius=UDim.new(0,6)
        local function rf() if getSt() then b.Text=onCl.text; b.BackgroundColor3=onCl.bg else b.Text=offCl.text; b.BackgroundColor3=offCl.bg end end; rf()
        b.MouseButton1Click:Connect(function() onTog(); rf() end); y=y+38
    end
    local function addSec(txt) local l=Instance.new("TextLabel",content); l.Size=UDim2.new(1,-PAD*2,0,16); l.Position=UDim2.new(0,PAD,0,y); l.BackgroundTransparency=1; l.Text=txt; l.TextColor3=Color3.fromRGB(130,130,150); l.Font=Enum.Font.GothamBold; l.TextSize=10; l.TextXAlignment=Enum.TextXAlignment.Center; y=y+20 end
    local function updPwr() if isActive then powerBtn.BackgroundColor3=Color3.fromRGB(60,200,60); powerBtn.Text="⏻" else powerBtn.BackgroundColor3=Color3.fromRGB(200,60,60); powerBtn.Text="⏸" end end
    powerBtn.MouseButton1Click:Connect(function() isActive=not isActive; if isActive then task.spawn(function() task.wait(0.1); captureAllowedHats() end) else removeAllHats() end; updPwr() end)
    ...
    addModeRow("🔀 Axis",{{key="Y",text="Y Halo"},{key="X",text="X Tumble"},{key="Z",text="Z Fan"}},function() return ORBIT_MODE end,function(k) ORBIT_MODE=k end)
    ...
    -- OUTER RING 2 (with 3-AXIS WING MODE)
    addSec("── Outer Ring 2 ──")
    ...
    addModeRow("🔀 Axis",{{key="Y",text="Y Halo"},{key="X",text="X Tumble"},{key="Z",text="Z Fan"}},function() return OUTER2_MODE end,function(k) OUTER2_MODE=k end)
    ...
    addSlider("↕︁ Height",-10000,10000,OUTER2_HEIGHT,0.5,function(v) OUTER2_HEIGHT=v end)
    addSlider("💫 Spin",-10000,10000,OUTER2_SPIN,0.5,function(v) OUTER2_SPIN=v end)
    ...
    addSlider("X Speed",1,180,WING_SPEED_X,1,function(v) WING_SPEED_X=v end)
    ...
    addSlider("Y Speed",1,180,WING_SPEED_Y,1,function(v) WING_SPEED_Y=v end)
    ...
    addSlider("Z Speed",1,180,WING_SPEED_Z,1,function(v) WING_SPEED_Z=v end)
    ...
    addModeRow("🔀 Axis",{{key="Y",text="Y Halo"},{key="X",text="X Tumble"},{key="Z",text="Z Fan"}},function() return OUTER3_MODE end,function(k) OUTER3_MODE=k end)
    ...
    addSlider("↕︁ Height",-10000,10000,OUTER3_HEIGHT,0.5,function(v) OUTER3_HEIGHT=v end)
    ...
    addModeRow("🔀 Axis",{{key="Y",text="Y Halo"},{key="X",text="X Tumble"},{key="Z",text="Z Fan"}},function() return OUTER4_MODE end,function(k) OUTER4_MODE=k end)
    ...
    addSlider("↕︁ Height",-10000,10000,OUTER4_HEIGHT,0.5,function(v) OUTER4_HEIGHT=v end)
    ...
    addModeRow("🔀 Axis",{{key="Y",text="Y Halo"},{key="X",text="X Tumble"},{key="Z",text="Z Fan"}},function() return INNER_ORBIT_MODE end,function(k) INNER_ORBIT_MODE=k end)
    ...
    local function mkBtn(txt,bg,cb) local b=Instance.new("TextButton",content); b.Size=UDim2.new(1,-PAD*2,0,34); b.Position=UDim2.new(0,PAD,0,y); b.Text=txt; b.BackgroundColor3=bg; b.TextColor3=Color3.new(1,1,1); b.Font=Enum.Font.GothamBold; b.TextSize=12; b.ZIndex=3; Instance.new("UICorner",b).CornerRadius=UDim.new(0,7); b.MouseButton1Click:Connect(cb); y=y+38; return b end
    local capBtn=mkBtn("🎩 Capture All Hats",Color3.fromRGB(70,130,240),function() captureAllowedHats(); capBtn.BackgroundColor3=Color3.fromRGB(50,190,70); capBtn.Text="✅ Captured!"; task.delay(0.8,function() if capBtn and capBtn.Parent then capBtn.BackgroundColor3=Color3.fromRGB(70,130,240); capBtn.Text="🎩 Capture All Hats" end end) end)
    mkBtn("🗑️ Remove All",Color3.fromRGB(200,55,55),function() removeAllHats(); if capBtn and capBtn.Parent then capBtn.Text="🎩 Capture All Hats" end end)
    mkBtn("👁️ Toggle GUI [RightShift]",Color3.fromRGB(55,55,65),function() main.Visible=not main.Visible end)

    content.Size=UDim2.new(1,-8,0,y+10); scrollFrame.CanvasSize=UDim2.new(0,0,0,y+10)
...
-- INIT
task.spawn(function() local ok,err=pcall(createGUI); if not ok then warn("GUI Error: "..tostring(err)) end end)
task.spawn(function() task.wait(1); captureAllowedHats() end)

print("🔥 Hat Orbit v9.9.8 Ready — 3-Axis Wing Mode on Outer Ring 2 — RightShift to toggle GUI")
